...
"""
function yahoo(sym::AbstractString = "^GSPC", opt::YahooOpt = YahooOpt(), ssl_verification = true)
    host = rand(["query1", "query2"])
    url  = "https://$host.finance.yahoo.com/v8/finance/chart/$sym"
    res  = HTTP.get(url, query = opt, require_ssl_verification = ssl_verification)
...
- ons()   which is a wrapper to download financial and economic time series data from the Office for National Statistics (ONS).
"""
function fred(data::AbstractString="CPIAUCNS", ssl_verification = true)
    url = "http://research.stlouisfed.org/fred2/series/$data/downloaddata/$data.csv"
    res = HTTP.get(url, require_ssl_verification = ssl_verification)
    @assert res.status == 200
...
"""
function ons(timeseries::AbstractString = "L522", dataset::AbstractString = "MM23", ssl_verification = true)
